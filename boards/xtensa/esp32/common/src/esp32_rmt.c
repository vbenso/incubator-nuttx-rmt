/****************************************************************************
 * boards/xtensa/esp32/esp32-devkitc/src/esp32_rmt.c
 *
...
static int rmt_open(FAR struct file *filep)
{
  ...
  rmtinfo("%d", __LINE__);
  FAR struct rmt_dev_s *parent_dev = (struct rmt_dev_s*)dev_data->parent_dev;
  int ret;
  irqstate_t flags;
  ...
  if (dev_data->open_count == 0)
    {
      ...
      rmtinfo("Setting thr limit at %08X to %d", reload_addr, dev_data->reload_thresh);
                        ...
                        size_t len)
{
  ...
  modifyreg32(
    RMT_INT_ENA_REG,
    0,
    RMT_CHN_TX_THR_EVENT_INT_ENA(dev_data->ch_idx)|RMT_CHN_TX_END_INT_ENA(dev_data->ch_idx));
